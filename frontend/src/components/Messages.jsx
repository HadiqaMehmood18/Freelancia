...
export default function Message({ selectedMessage }) {
    ...
    const sendMessageAndDisplay = () => {
        if (selectedFile) {
            ...
            formData.append('receiverId', messages.messages.withInfo._id)
            formData.append('chatId', selectedMessage)

            // Get token from localStorage
            const token = localStorage.getItem("token");
            console.log("Sending file upload with token:", token);

            // Assuming you have an API endpoint to upload files and send message
            fetch('http://localhost:1900/chat/upload', {
                method: 'POST',
                body: formData,
                headers: {
                ...
                if (data.status === 200) {
                    // Emit socket event with file message info
                    socket.current.emit('sendMessage', {
                        senderId: id,
                        receiverId: messages.messages.withInfo._id,
                        text: '', // No text for file message
                        chatId: selectedMessage,
                        file: selectedFile.name
                    });
                    setSelectedFile(null)
                    dispatch(conversationMessages(selectedMessage)).unwrap()
        ...
        <div className="selectedMessage">
            ...
            <div className="sendBox">
                <div className="inputbox">
                    <div className="svg" onClick={handlePaperclipClick} style={{ cursor: 'pointer' }}>
