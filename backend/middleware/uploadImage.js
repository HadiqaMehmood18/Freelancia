...
const createChatFileUpload = (req, res, next) => {
  const storage = multer.diskStorage({
    destination: (req, file, cb) => {
      if (!existsSync("uploads/ChatFiles")) {
        mkdirSync("uploads/ChatFiles", { recursive: true });
      }
  ...
  // Accept all file types for chat uploads
